...
'''
Copied from @alexandrosstergiou https://github.com/alexandrosstergiou/keras-DepthwiseConv3D/blob/master/DepthwiseConv3D.py
...
class DepthwiseConv3D(Conv3D):
    ...
    def build(self, input_shape):
        ...
        self.input_spec = InputSpec(
            ndim=5, axes={channel_axis: self.input_dim})
        self.built = True
    ...
    def compute_output_shape(self, input_shape):
        ...
        depth = utils.conv_utils.conv_output_length(depth, self.kernel_size[0],
                                                    self.padding,
                                                    self.strides[0])

        rows = utils.conv_utils.conv_output_length(rows, self.kernel_size[1],
                                                   self.padding,
                                                   self.strides[1])

        cols = utils.conv_utils.conv_output_length(cols, self.kernel_size[2],
                                                   self.padding,
                                                   self.strides[2])
