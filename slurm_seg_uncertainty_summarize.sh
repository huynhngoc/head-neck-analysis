...
#SBATCH --output=outputs/uncertainty-sum-%A-%a.out
#SBATCH --error=outputs/uncertainty-sum-%A-%a.out
...
rm -rf $TMPDIR/ray/*
singularity exec --nv deoxys-2023-feb-fixed.sif python -u run_uncertainty_summarize.py $1 $PROJECTS/ngoc/segmentation --dropout_rate $2 --iter $SLURM_ARRAY_TASK_ID
